...
public class World {
	...
	public void render(Camera camera) {
		...
		/*
		 * for (var block : blocks) { //if (block.isSelected()) }
		 */

		// var pickRay = camera.getPickRay(Gdx.graphics.getWidth() / 2,
		// Gdx.graphics.getHeight() / 2);
		camera.update();
		var pickRay = camera.getPickRay(Gdx.graphics.getWidth() / 2, Gdx.graphics.getHeight() / 2);
		// System.out.println(pickRay.direction);
		// var end = pickRay.origin.cpy().add(pickRay.direction.scl(12)).sub(0, 0, 2);
		Vector3 end = new Vector3();
		pickRay.getEndPoint(end, 10).sub(0, 0, 2);
		// System.out.println(end + ", Origin: " + pickRay.origin + ", direction: " +
		// pickRay.direction);
		// var end = camera.position.cpy().add(camera.direction).sub(0, 0, -2);
